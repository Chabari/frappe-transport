...
frappe.ui.form.on('Requested Payments', {
	...
	refresh: function (frm) {
		...
		if(cur_frm.doc.previous_requested_funds != undefined){
			cur_frm.doc.previous_requested_funds.forEach(function(row){
			
				if(row.request_status == "Approved" && row.request_currency == 'USD')
				{
...
cur_frm.cscript.disburse_request = function (frm) {
	...
	if (selected['accounts_approval']) {
		...
		frappe.confirm(
			'Confirm: Disbursess selected requests?',
			function () {
