...
frappe.ui.form.on('Vehicle Trip', {
    setup: function(frm) {
        ...
        frm.set_query('item_code','main_fuel_request', function(doc) {
        //   return {
        //     filters: {
        //       item_group: 'Service'
        //     }
    ...
    refresh: function (frm) {
        ...
        if (frm.doc.trip_completed == 0) {
            if(frm.doc.custom_transport_type == "Town Trip"){

                frm.add_custom_button(__("Complete Trip"), function () {
                
                var d = new frappe.ui.Dialog({
                    title: __('Enter Trip End Date'),
                    fields: [
                        {
                            "label" : "Trip End Date",
                            "fieldname": "end_date",
...
frappe.ui.form.on("Requested Funds Details", "request_amount", function(frm, cdt, cdn) {
    frappe.db.get_single_value('Transport Settings', 'default_expense_account')
    .then(default_expense_account => {
        if(default_expense_account != null){
            frappe.model.set_value(cdt, cdn, 'expense_account', default_expense_account);
        }
    });
    frappe.db.get_single_value('Transport Settings', 'default_payable_account')
    .then(default_payable_account => {
        if(default_payable_account != null){
            frappe.model.set_value(cdt, cdn, 'payable_account', default_payable_account);
        }
    });
